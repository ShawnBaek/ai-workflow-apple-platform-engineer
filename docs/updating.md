...
Use the same installation method and scope that created your active copy.
First identify its source revision, resolved skill path and any local edits.
Finish work using that copy before replacing it; preserve customizations and
the previous revision for rollback. Fetching this repository alone does not
update a copied or versioned installation.
...
1. Check the actual loaded skill paths and source revision, not only the README
   version label. Confirm one discoverable copy of each selected skill.
2. For the bundled Swift runtime, build the changed executable once with the
   selected full Xcode and run the relevant [verification](verification.md).
...
3. If a private coordinator/runtime binding is configured, follow the explicit
   [runtime migration procedure](../skills/agent-harness/references/swift-verification.md)
