...
|---|---|---|
| **App Store media freshness:** target release with an older installed app and mixed `latest/` artifacts; undersized preview frames; contrast PR-only recording and a device IPA for Simulator | Bind screenshots and recordings to the resolved source/version/build and installed variant. Recapture stale/unknown assets, use native stills for required slots, preserve PR-only scope and verify Simulator variant parity. | Partial: [App Store media checks](evidence/app-store-media-check.md) include four proposed-action cases and executed synthetic PNG validation, video trim and frame extraction. Real app/build capture and upload remain untested. |
| **First-run setup and upgrade:** missing `gh`/full Xcode with no harness; active consumers with zero leases; selective copies with no commit metadata; contrast no-change inspection and a README typo | Install only selected, authorized dependencies; continue independent setup while answers are pending. Preserve installation scope and active grants, activate only after quiescence, and keep lightweight setup free of harness prerequisites. Report installed/configured/exposed/capability-verified separately. | Partial: [first-run setup checks](evidence/first-run-setup-check.md) cover proposed-action scenarios, independent review and real read-only tool discovery. Clean installation, activation and representative app execution remain untested. |
...
| **Figma parity routing and verification:** a Figma node URL plus "add snapshot tests"; the nearest confusing case without a design source; an existing catalog asset with a design-like name | Route the snapshot/parity part to `figma-golden-testing` and keep `apple-platform-testing` for the no-Figma case. Use exported artwork after rendering and comparing it; never reuse an asset by name or substitute an SF Symbol for a design-system icon. Capture with the documented SwiftUI strategy, match dimensions at the source, and verify color/size/position/hit area with numbers. | Partial: [author decision replay](evidence/figma-parity-routing-check.md) for the three requests before and after the instruction change. No fresh-agent routing run or real Figma-to-Simulator comparison. |
