...
Claude Code **plugin marketplace** for indie developers shipping **Apple-platform native apps end-to-end** — from blank Xcode project to App Store submission, on your own, without a design or DevOps team.

Eight agents that share one job: kill the time-sinks that make shipping native apps as a solo dev painful.

## The eight agents

| Agent | When to invoke | What it owns |
|-------|----------------|--------------|
| [`apple-platform-ui`](plugins/apple-platform-ui/README.md) | "Build me a screen", "design this view", "SwiftUI / UIKit" | UI implementation in SwiftUI/UIKit, grounded in Apple HIG |
| [`apple-platform-performance`](plugins/apple-platform-performance/README.md) | "App is janky", "scroll stutters", "launch is slow", "Instruments shows…" | Hangs / hitches / launch / body-cost diagnosis with 23 Effective-style items |
| [`xcodebuild`](plugins/xcodebuild/README.md) | "Build", "run on sim", "screenshot the screen", "attach debugger" | Xcode builds + simulator + UI automation via XcodeBuildMCP |
...
| [`app-store-connect`](plugins/app-store-connect/README.md) | "TestFlight", "submit", "store metadata", "crash reports" | App Store Connect ops via the asc CLI |
...
```
   apple-platform-ui         apple-platform-performance        xcodebuild
        ↓                         ↓                       ↓
   SwiftUI code             Hangs / hitches /         Build + run on
   with mocks,              launch / body cost,       simulator;
   Light/Dark/XXL    →      Apple Instruments    →    capture logs;
   previews                 + XCTMetric tests         UI tests
                                                          ↓
...
- **Sub-docs** for big agents live in `plugins/<short-name>/agents/agent-<name>/<topic>.md`. The router agent file stays small (overview + quick-reference table) and tells the subagent to `Read` the matching sub-doc when a topic comes up. Used today by `app-website` (6 sub-docs), `apple-platform-ui` (2 sub-docs: keyboard, launch-screen), and `apple-platform-performance` (5 sub-docs: parts 1–5).
