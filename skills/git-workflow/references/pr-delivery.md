...
3. Prepare a short PR body from the repository template: problem/result, checks with outcomes, and accessible proof. Link an existing Issue when the repository policy or accepted task uses one; do not create or require an Issue for a plain Git workflow unless another selected workflow requires it. Link the detailed evidence report rather than copying its full matrix or usage accounting.
