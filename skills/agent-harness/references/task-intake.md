...
Once material questions and required execution gates are settled, start the first ready implementation assignment. Give workers the accepted brief, relevant decisions, exact inputs/owned paths, expected output and verification target. Do not ask each worker to repeat intake, research, an ADR or full host setup; revalidate only facts its action depends on. Workers must not invent competing product requirements. When the user changes the request, revisit only dependent work.

For a task-to-PR request, carry the delivery target into the assignment. Resolve the app checkout and inspect its configured remote/default branch; the skill collection's upstream is only for collection changes or an authorized skill report. Ask for account or destination confirmation only where policy requires it and the answer is missing. Group those known missing facts early, continue independent work, and reserve any required immediate commit approval for the prepared diff. A worker finishing code hands back to the lead for delivery; it does not reset the task to local-only. An explicit local-only task still stops locally.
