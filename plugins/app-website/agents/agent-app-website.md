...
For depth on any topic, `Read` the matching file under [`plugins/app-website/agents/agent-app-website/`](agent-app-website/):

| When the developer asks about… | Open |
|---|---|
| Verifying the site in a real browser + installing Playwright MCP on Claude Code / VS Code / Codex | [`playwright-verify.md`](agent-app-website/playwright-verify.md) |
| Which SwiftUI-For-Web APIs exist, the escape-hatch helpers, the importmap requirement | [`api-reference.md`](agent-app-website/api-reference.md) |
| The 5-section detailed spec + Gridlover rhythm + file layout + performance budget | [`sections.md`](agent-app-website/sections.md) |
| Mobile / tablet / desktop breakpoints, stepped tokens, `.row-wrap`, `prefers-reduced-motion` | [`responsive.md`](agent-app-website/responsive.md) |
| 3D Apple device models (iPhone / iPad / MacBook / Watch), `<model-viewer>`, USDZ vs GLB sources | [`3d-devices.md`](agent-app-website/3d-devices.md) |
| GitHub Pages deploy + custom domain (`yourapp.com`) + DNS + HTTPS + alternative hosts | [`deploy.md`](agent-app-website/deploy.md) |
...
Detail for each section is in [`sections.md`](agent-app-website/sections.md). Don't add a 4th feature, a nav bar, an email-capture form, or a chat widget.
...
2. **Generate the full file layout** (per [`sections.md`](agent-app-website/sections.md)) with all 5 sections wired up, `theme.js` + `typography.js` + `helpers.js` modules, the responsive CSS tokens, the `index.html` shell **with importmap and a cache-buster on `./main.js`**.
3. **Insert TODOs** where you're missing real content (screenshots, App Store URL).
4. **Start a no-cache local server** (per [`playwright-verify.md`](agent-app-website/playwright-verify.md) — Python's default `http.server` caches; use the `tools/nocache-server.py` snippet).
...
7. **Iterate until everything renders + works.** Cache issues will bite you (see [`playwright-verify.md`](agent-app-website/playwright-verify.md) → "Cache discipline"). Close the tab and re-open if a hard refresh doesn't pick up changes.
...
- [ ] iPhone-framed screenshots (PNG bezel composite, or `<model-viewer>` per [`3d-devices.md`](agent-app-website/3d-devices.md)) — no raw screenshots floating in space.
...
- **Invent modifiers that don't exist** (`Link`, `.className`, `.style`, `.ariaLabel`, `.loading`, `HStack({ wrap: true })`) — see [`api-reference.md`](agent-app-website/api-reference.md).
...
- **Trust Python's `http.server`** during development. It doesn't send `Cache-Control: no-cache`, and you'll spend an hour wondering why your edit didn't apply. Use the no-cache server in [`playwright-verify.md`](agent-app-website/playwright-verify.md).
...
[`examples/NotesJournalWebsite/`](../../../examples/NotesJournalWebsite/) — a complete buildable one-pager generated by this agent. All 5 sections wired up, real SwiftUI-For-Web modifiers, Gridlover rhythm, dark mode, scroll-driven reveal + parallax, no third-party scripts, footer credit. Verified to render with clean console via Chrome MCP. Use it as the shape to match.
