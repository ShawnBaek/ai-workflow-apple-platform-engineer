...
For depth on any topic, `Read` the matching file under [`agent-figma-bridge/`](agent-figma-bridge/):
...
- Generate code from a "too big" Figma frame in one MCP call. Always check size first and select a smaller node if needed ([`generate-from-frame.md`](agent-figma-bridge/generate-from-frame.md)).
- Recommend deprecated `Figma.connect` syntax — always use the current SwiftUI Code Connect API ([`code-connect.md`](agent-figma-bridge/code-connect.md)).
