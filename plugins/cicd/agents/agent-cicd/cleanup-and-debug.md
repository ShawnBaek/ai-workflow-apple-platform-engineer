...
The cicd agent routes failures to specialist agents:

| Failure pattern in log | Route to |
|---|---|
| `xcodebuild` exit ≠ 0; compile errors, link errors, scheme not found, signing failures | [`xcodebuild`](../../../xcodebuild/agents/agent-xcodebuild.md) |
| `asc` exit ≠ 0; submission rejected, build processing stuck, certificate expired | [`app-store-connect`](../../../app-store-connect/agents/agent-app-store-connect.md) |
| Simulator boot failure, screen capture fail, UI automation tap missed | [`xcodebuild`](../../../xcodebuild/agents/agent-xcodebuild.md) (has simulator + ui-automation tools) |
| Unit / UI test failure | Read `.xcresult` → identify the test → flag the underlying code change |
| Test perf regression (XCTMetric baseline exceeded) | [`apple-platform-performance`](../../../apple-platform-performance/agents/agent-apple-platform-performance.md) |
| Hang report from CI (`MetricKit` payload) | [`apple-platform-performance`](../../../apple-platform-performance/agents/agent-apple-platform-performance.md) → Part II (Hangs) |
